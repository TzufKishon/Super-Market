...
Address* loadAddressFromCompressFile(FILE* fp)
{
	Address* superAddress = (Address*)malloc(sizeof(Address));
	BYTE data[1];
	if (fread(&data, sizeof(BYTE), 1, fp) != 1)
		return NULL;

	superAddress->num = data[0];

	superAddress->street = readAddressFromCompressFile(fp, "error read street name\n");
	if (!superAddress->street)
		return NULL;

	superAddress->city = readAddressFromCompressFile(fp, "error read city name\n");
	if (!superAddress->city)
		return NULL;
