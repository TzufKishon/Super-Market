...
int		initSuperMarket(SuperMarket* pMarket, int fileType, const char* fileName,
	const char* customersFileName)
{
	...
	{
		if (loadSuperMarketFromCompressFile(pMarket, fileName, customersFileName) == 1)
		{
			printf("Supermarket successfully loaded from files\n");
			return 1;
		}
...
{
	Customer cust;
	CHECK_RETRUN_0(initCustomer(&cust));
